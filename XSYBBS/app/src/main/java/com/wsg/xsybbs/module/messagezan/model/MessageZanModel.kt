package com.wsg.xsybbs.module.messagezan.model
...
class MessageZanModel {
    var message: MessageZanMessage? = null

    fun getZanInfo(mZanList: MutableLiveData<List<Zan>>, mZanMessage: MutableLiveData<MessageZanMessage>) {
                                ...
                                override fun done(p0: MutableList<Zan>?, p1: BmobException?) {
                                    if (p1 == null) {
                                        if (p0?.size!! > 0) {
                                            ...
                                            message = MessageZanMessage(MessageZanMessage.CODE_NO_ZAN, MessageZanMessage.MESSAGE_NO_ZAN)
                                            mZanMessage.postValue(message)
                                        }
                                    } else {
                                        message = MessageZanMessage(MessageZanMessage.CODE_FAIL, MessageZanMessage.MESSAGE_FAIL)
                                        mZanMessage.postValue(message)
                                    }
                                }
                            })
                        } else {
                            message = MessageZanMessage(MessageZanMessage.CODE_NO_NOTE, MessageZanMessage.MESSAGE_NO_NOTE)
                            mZanMessage.postValue(message)
                        }
                    } else {
                        message = MessageZanMessage(MessageZanMessage.CODE_FAIL, MessageZanMessage.MESSAGE_FAIL)
