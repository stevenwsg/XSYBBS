package com.wsg.xsybbs.module.messagecomment.model
...
class MessageCommentModel {
    ...
    fun getCommentInfo(commentList: MutableLiveData<List<Comment>>,
                       commentMessage: MutableLiveData<MessageCommentMessage>) {
                ...
                override fun done(p0: MutableList<Note>?, p1: BmobException?) {
                    if (p1 == null) {
                        if (p0?.size!! > 0) {
                                ...
                                override fun done(p0: MutableList<Comment>?, p1: BmobException?) {
                                    if (p1 == null) {
                                        if (p0?.size!! > 0) {
                                            ...
                                            message = MessageCommentMessage(MessageCommentMessage.CODE_NO_COMMENT, MessageCommentMessage.MESSAGE_NO_COMMENT)
                                            commentMessage.postValue(message)
                                        }

                                    } else {
                                        message = MessageCommentMessage(MessageCommentMessage.CODE_FAIL, MessageCommentMessage.MESSAGE_FAIL)
                                        commentMessage.postValue(message)
                                    }
                                }
                            })
                            ...
                            message = MessageCommentMessage(MessageCommentMessage.CODE_NO_NOTE, MessageCommentMessage.MESSAGE_NO_NOTE)
                            commentMessage.postValue(message)
                        }
                    } else {
                        message = MessageCommentMessage(MessageCommentMessage.CODE_FAIL, MessageCommentMessage.MESSAGE_FAIL)
